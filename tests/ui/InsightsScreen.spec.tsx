...
describe('InsightsScreen', () => {
  ...
  it('uses one caffeine-detail hierarchy with 2W selected, without Summary, Trends, or History controls', async () => {
    ...
    expect(screen.getByRole('button', { name: '2W' })).toHaveProp(
      'accessibilityState',
      { selected: true },
    );
    expect(
    ...
    expect(
      screen.queryByRole('button', { name: 'Trends' }),
    ).not.toBeOnTheScreen();
    expect(
  ...
  it('changes the selected headline, chart range, and trend content together', async () => {
    ...
    expect(screen.getByText('90 mg/day')).toBeOnTheScreen();
    ...
    expect(screen.getByText('80 mg/day')).toBeOnTheScreen();
  ...
  it('uses trailing Share and an Options hierarchy instead of the default Settings affordance', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Show All Data' }),
    ).toBeOnTheScreen();
  });
  ...
  it('keeps an empty period honest and exposes its chart availability in the accessibility summary', async () => {
    ...
    expect(
      screen.getByText('No caffeine data for this range.'),
    ).toBeOnTheScreen();
    expect(
      screen.getByLabelText(
        /Caffeine intake, 14 days.*No caffeine data is available/,
      ),
    ).toBeOnTheScreen();
  });
  ...
  it('shows readable Share and export failures and opens the focused caffeine history route', async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText('Unable to share insights. Share unavailable'),
      ).toBeOnTheScreen(),
    );
    await user.press(screen.getByRole('button', { name: 'Export CSV' }));
    await waitFor(() =>
      expect(
        screen.getByText('Unable to export insights. Share unavailable'),
      ).toBeOnTheScreen(),
    );
  ...
  it('uses an asymmetric primary-left layout on a wide iPad window', async () => {
    ...
    expect(screen.getByTestId('insights-primary-column')).toHaveStyle({
      width: 600,
    });
    expect(screen.getByTestId('insights-supporting-column')).toHaveStyle({
      width: 540,
    });
  });
});
